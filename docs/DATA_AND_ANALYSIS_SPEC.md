...
MVP 使用标准 CSV 测试数据，并支持本地上传来自问卷星、腾讯问卷和 App 问卷导出的售后反馈。
...
| 联系方式 | 可选 | 仅允许脱敏后的回访线索，MVP 不保存真实个人信息 |
...
- 标准 CSV 模板内置 10-20 条可导入测试反馈，便于首次验证字段解析。
- 后续压测数据至少包含 1 个千级样本项目，用于验证主机厂一周几百到几千条售后反馈的真实规模。
- 覆盖问卷星、腾讯问卷、App 内问卷。
- 覆盖 4 个区域。
- 覆盖多个城市和服务中心。
- 覆盖 6 类服务环节。
